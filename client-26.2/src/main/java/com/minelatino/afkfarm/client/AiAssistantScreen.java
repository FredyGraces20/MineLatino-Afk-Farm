...
public final class AiAssistantScreen extends Screen {
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        graphics.text(font, title, left + 9, 13, 0xFFEAFBFF, false);
        graphics.text(font, controller.accountLabel(), left + 9, 26, 0xFF9DB1BF, false);
        String connection = controller.connectionLabel();
        graphics.text(font, connection, left + panelWidth - 9 - font.width(connection), 26,
                connection.startsWith("Error") ? 0xFFFF6B7A : 0xFF61E7B7, false);
        renderHistory(graphics);
        super.extractRenderState(graphics, mouseX, mouseY, delta);
        if (controller.busy()) graphics.text(font, "La IA está escribiendo…", left + 10, historyBottom - 12, 0xFF7DEBFF, false);
        String error = controller.error();
        if (!error.isBlank()) graphics.centeredText(font, font.plainSubstrByWidth(error, panelWidth - 22), width / 2, historyBottom - 12, 0xFFFF8B94);
        if (System.currentTimeMillis() < copiedUntil) graphics.centeredText(font, copied, width / 2, 12, 0xFFFFC857);
    }
    ...
    private void renderHistory(GuiGraphicsExtractor graphics) {
        ...
        for (AiAssistantController.Message message : messages) {
            ...
            if (y + h >= historyTop && y <= historyBottom) {
                ...
                graphics.text(font, user ? "Tú" : "MineLatino IA", x + 8, y + 5, user ? 0xFF7DEBFF : 0xFFBDA8FF, false);
                int lineY = y + 16;
                for (FormattedCharSequence line : lines) { graphics.text(font, line, x + 8, lineY, 0xFFF1F5F7, false); lineY += 10; }
                hits.add(new MessageHit(x, Math.max(y, historyTop), x + bubbleWidth, Math.min(y + h, historyBottom), message.content()));
            }
            y += h + 6;
        }
        if (messages.isEmpty()) {
            graphics.centeredText(font, "Pregunta sobre AFK Farm o MineLatino", width / 2, historyTop + 20, 0xFF8FA3B0);
            graphics.centeredText(font, "Haz clic en un mensaje para copiarlo", width / 2, historyTop + 34, 0xFF637681);
        }
